...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('warehouses_products') . ".product_id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name, categories.name as catename, " . $this->db->dbprefix('products') . ".cf1 as cf1, cf2 as cf2, unit as unit, " . $this->db->dbprefix('warehouses_products') . ".quantity as quantity, price as price", FALSE)
                ->from('warehouses_products')
                ->join('products', 'products.id=warehouses_products.product_id', 'left')
                ->join('categories', 'products.category_id=categories.id', 'left')
                ->where('warehouses_products.warehouse_id', $warehouse_id)
                ->where('warehouses_products.item_id', NULL)
                ->where('warehouses_products.quantity !=', 0)
                ->group_by("warehouses_products.product_id");
        } else {
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name, categories.name as catename, " . $this->db->dbprefix('products') . ".cf1 as cf1, cf2 as cf2,unit as unit ,price as price", FALSE)
                ->from('products')
                ->join('categories', 'products.category_id=categories.id', 'left')
    ...
    function add($id = NULL)
    {
        ...
        $this->form_validation->set_rules('unit', lang("unit"), 'trim');
        $this->form_validation->set_rules('price', lang("price"), 'trim|numeric');
        $this->form_validation->set_rules('cf3', lang("Nhân công"), 'trim|numeric');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $attrconfig=array();
            $it_id=$this->input->post('attr_id');
            $it_item=$this->input->post('attr_item');

            $it_quantity=$this->input->post('attr_quantity');

            for ($i=0; $i <count($it_item) ; $i++) {
                $std=new stdClass();
                $std->id=$it_id[$i];
                $std->item=$it_item[$i];
                $std->quantity=$it_quantity[$i];

                $attrconfig[]=$std;
            }
            $attr_name=$this->input->post('attr_name');
            $attr_percent=$this->input->post('attr_percent');
            $attr_var_id =$this->input->post('attr_var_id');
            $cf4=array();
            for ($i=0; $i < count($attr_name); $i++) {
                $arr=array();
                $arr['attr_id']=$attr_var_id[$i];
                $arr['attr_name']=$attr_name[$i];
                $arr['attr_percent']=$attr_percent[$i];
                $cf4[]=$arr;
            }
            $data = array(
                ...
                // 'quantity' => $this->input->post('wh_qty_1'),
                ...
                'cf1' => $this->input->post('cf1'),//Quy cach
                'cf2' => $this->input->post('cf2'),//Trong luong
                'cf3' => $this->input->post('cf3'),//Nhân cong
                'cf4' => ($cf4)? json_encode($cf4):NULL,//Giai doan
                'cf5' => (!empty($attrconfig))?json_encode($attrconfig):NULL,//Cau thanh
                'cf6' => $this->input->post('cf6'),
                'cf7' => $this->input->post('cf7'),
                'cf8' => $this->input->post('cf8'),
                'quantity_config' => $this->input->post('quantity_config'),
            );
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $warehouse_qty, $photos)) {
            ...
            $this->data['items'] = $this->items_model->getAllItems();
            $this->data['variants'] = $this->site->getAllVariants();
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data,$warehouse_qty,$photos)) {
            ...
            $product->cf4=json_decode($product->cf4);
            $product->cf5=json_decode($product->cf5);
            $this->data['product'] = $product;
            // echo "<pre>";
            // var_dump($product->cf4);die();
            $this->data['items'] = $this->items_model->getAllItems();
            $this->data['variants'] = $this->site->getAllVariants();
    ...
    function import_csv_zzz()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('code', 'name', 'category_code', 'unit', 'cost', 'price', 'cf1', 'cf2', 'cf3', 'cf6', 'cf7', 'cf8','quantity_config');
// var_dump($arrResult);die();
                ...
                //     echo trim($csv_pr['category_code'])."<br>";
                // }
                // die();
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if ($catd = $this->products_model->getCategoryByCode(trim($csv_pr['category_code']))) {
                        ...
                        $cf7[] = trim($csv_pr['cf7']);
                        $cf8[] = trim($csv_pr['cf8']);
                        $quantity_config[] = trim($csv_pr['quantity_config']);
                    } else {
            ...
            $ikeys = array('code', 'name', 'category_id', 'unit', 'cost', 'price', 'cf1', 'cf2', 'cf3', 'cf6', 'cf7', 'cf8', 'quantity_config');

            $items = array();
            foreach (array_map(null, $pr_code, $pr_name, $pr_cat, $pr_unit, $pr_cost, $pr_price, $cf1, $cf2, $cf3, $cf6, $cf7, $cf8, $quantity_config) as $ikey => $value) {
    ...
    function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if ($catd = $this->products_model->getCategoryByCode(trim($csv_pr['category_id']))) {
                        // $pr_code[]  = trim($csv_pr['code']);
                        ...
                        $this->session->set_flashdata('error', lang("check_category_code") . " (" . $csv_pr['category_id'] . "). " . lang("category_code_x_exist") . " " . lang("line_no") . " " . $rw);
        ...
        if ($this->form_validation->run() == true && $this->products_model->add_products2($items)) {
            $this->session->set_flashdata('message', lang("products_added"));
            redirect('products');
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
    ...
    function modal_view($id = NULL)
    {
        ...
        $pr_details->cf4=json_decode($pr_details->cf4);
        $pr_details->cf5=json_decode($pr_details->cf5);
    ...
    {
        if ($rows = $this->products_model->getProductByID($id)) {
