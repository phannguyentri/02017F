...
class Reports extends MY_Controller
{
    ...
    function report_ycsx ()
    {


        $data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        
        $a =  $this->reports_model->Getycsx();
        
        // $b = $this->reports_model->getT();
        // $c = $this->reports_model->getC();
        // $a['t'] = $b;
        // $a['c'] = $c;

        // $a['loinhuangop'] = $a[0]->total_payment - $a[0]->total_pur;
       
        // $a['tongloinhuan'] = $a['loinhuangop']+$a['t']->price_t - $a['c']->price_c;

    
       

        $this->data['monthly_sales'] = $a;
  
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('reports')));
        $meta = array('page_title' => lang('reports'), 'bc' => $bc);
        $this->page_construct('reports/report_ycsx', $meta, $this->data);
    }
