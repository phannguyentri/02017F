...
class Purchases extends MY_Controller
{
    ...
    function addItemQuickEnquiry(){
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $row->expiry = '';
                $row->qty = $row->quantity_balance;
                $row->quantity_balance = $row->quantity_balance;
                $row->discount = '0';
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => ($c+$r), 'item_id' => $row->item_id, 'label' => $row->product_name . " (" . $row->specification . ")", 'row' => $row, 'tax_rate' => $tax_rate, 'options' => $options);
                } else {
                    $pr[] = array('id' => ($c+$r), 'item_id' => $row->item_id, 'label' => $row->product_name . " (" . $row->specification . ")", 'row' => $row, 'tax_rate' => false, 'options' => $options);
                }
                $r++;
            }
            echo json_encode($pr);
    ...
    function getPurchases($warehouse_id = NULL)
    {
        ...
        $status = array('received', 'pending', 'ordered');
        if ($warehouse_id) {
            $this->datatables
...
//generate pdf and force to download
    ...
    function add($quote_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_name) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    $product_details = $this->purchases_model->getProductByNameS($item_name,$item_specification);
            ...
            $data = array('reference_no' => $reference,
                'enquiery' => $this->input->post('enquiery'),
                'parent_id' => $this->input->post('enquiery'),
                'date' => $date,
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $this->data['enquiries'] = $this->site->getAllEnquiry1();
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
