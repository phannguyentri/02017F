...
class Enquiry extends MY_Controller
{
    ...
    function getEnquiry($warehouse_id = NULL)
    {
        ...
        $detail_link = anchor('enquiry/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('enquiry_details'));

        $edit_link = anchor('enquiry/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_enquiry'));
        $pdf_link = anchor('enquiry/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $delete_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_enquiry") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('enquiry/delete/$1') . "'>"
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
            . lang('delete_enquiry') . "</a>";
        ...
        $status = array('approve', 'approval','done');
        if ($warehouse_id) {
            $this->datatables
                ->select("id, date, reference_no, grand_total,status")
                ->from('purchases')
            ...
            $this->datatables
                ...
                ->or_where('status','done')
                ->or_where('status','approval');
        }
    ...
    function modal_view($purchase_id = NULL)
    {
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->enquiry_model->getPurchaseByID($purchase_id);
        $this->sma->view_rights($inv->created_by, TRUE);
        $this->data['rows'] = $this->enquiry_model->getAllPurchaseItems($purchase_id);
        $this->data['supplier'] = $this->site->getCompanyByID($inv->supplier_id);
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $this->data['payments'] = $this->enquiry_model->getPaymentsForPurchase($purchase_id);
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['updated_by'] = $inv->updated_by ? $this->site->getUser($inv->updated_by) : NULL;

        $this->load->view($this->theme.'enquiry/modal_view', $this->data);
    ...
    function view($purchase_id = NULL)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('enquiry'), 'page' => lang('enquiry')), array('link' => '#', 'page' => lang('Xem yêu cầu mua hàng ').$purchase_id));
        $meta = array('page_title' => lang('Xem yêu cầu mua hàng ').$purchase_id, 'bc' => $bc);
        $this->page_construct('enquiry/view', $meta, $this->data);
...
//generate pdf and force to download
    ...
    function pdf($purchase_id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        ...
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['inv'] = $inv;
        $name = $this->lang->line("enquiry") . "_" . str_replace('/', '_', $inv->reference_no) . ".pdf";
        $html = $this->load->view($this->theme . 'enquiry/pdf', $this->data, TRUE);
        if ($view) {
            $this->load->view($this->theme . 'enquiry/pdf', $this->data);
        } elseif ($save_bufffer) {
     ...
     function add($quote_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_specification = $_POST['product_specification'][$r];
                ...
                if (isset($item_name) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {

                    $product_details = $this->enquiry_model->getProductByNameS($item_name,$item_specification);
        ...
        if ($this->form_validation->run() == true && $this->enquiry_model->addEnquiry($data, $products)) {
            $this->session->set_userdata('remove_polss', 1);
            $this->session->set_flashdata('message', $this->lang->line("enquiry_add"));
            redirect('enquiry');
        } else {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('enquiry'), 'page' => lang('enquiry')), array('link' => '#', 'page' => lang('add_enquiry')));
            $meta = array('page_title' => lang('add_enquiry'), 'bc' => $bc);
            $this->page_construct('enquiry/add', $meta, $this->data);
        }
    }
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->input->get('id')) {
         ...
         $this->data['purchase'] = $this->enquiry_model->getPurchaseByID($id);

        if($this->data['purchase']->status == 'done'){
            $this->session->set_flashdata('error', $this->lang->line("Không được phép sửa yêu cầu nhập hàng này"));
            redirect('enquiry');
        }
        ...
        if ($this->form_validation->run() == true && $this->enquiry_model->updatePurchase($id, $data, $products)) {
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line("enquiry_edit"));
            redirect('enquiry');
        } else {

            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['inv'] = $this->enquiry_model->getPurchaseByID($id);
            ...
            $inv_items = $this->enquiry_model->getAllPurchaseItems($id);

            $c = rand(100000, 9999999);

            foreach ($inv_items as $item) {

                $row = $this->site->getItemByID($item->item_id);
                ...
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->item . " (" . $row->specification . ")", 'row' => $row, 'tax_rate' => $tax_rate, 'options' => $options);
                } else {
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->item . " (" . $row->specification . ")", 'row' => $row, 'tax_rate' => false, 'options' => $options);
                }
                $c++;
            }
            ...
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('enquiry'), 'page' => lang('enquiry')), array('link' => '#', 'page' => lang('edit_enquiry')));
            $meta = array('page_title' => lang('edit_enquiry'), 'bc' => $bc);
            $this->page_construct('enquiry/edit', $meta, $this->data);
        }
    }
    ...
    function delete($id = NULL)
    {
        ...
        if ($this->enquiry_model->deletePurchase($id)) {
            if($this->input->is_ajax_request()) {
                echo lang("enquiry_deleted"); die();
            }
            $this->session->set_flashdata('message', lang('enquiry_deleted'));
            redirect('welcome');
        }
    }
    ...
    function suggestions()
    {
        ...
        $rows = $this->enquiry_model->getProductNames($term);
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                    $pr[] = array('id' => ($c+$r), 'item_id' => $row->id, 'label' => $row->item . " (" . $row->specification . ")", 'row' => $row, 'tax_rate' => $tax_rate, 'options' => $options);
                } else {
                    $pr[] = array('id' => ($c+$r), 'item_id' => $row->id, 'label' => $row->item . " (" . $row->specification . ")", 'row' => $row, 'tax_rate' => false, 'options' => $options);
                }
                $r++;
            }
            echo json_encode($pr);
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->enquiry_model->deletePurchase($id);
                    }
                    $this->session->set_flashdata('message', $this->lang->line("enquiry_deleted"));
                    redirect($_SERVER["HTTP_REFERER"]);
                }
