...
class Products_model extends CI_Model
{
    ...
    public function addProduct($data, $warehouse_qty, $photos)
    {
    ...
    public function add_products2($products = array())
    {
        if (!empty($products)) {
            foreach ($products as $product) {
    ...
    public function updateProduct($id, $data,$warehouse_qty, $photos)
    {
        ...
        if ($this->db->update('products', $data, array('id' => $id))) {
            ...
            if ($warehouse_qty && !empty($warehouse_qty)) {
                ...
                foreach ($warehouse_qty as $wh_qty) {
                    if($this->db->update('warehouses_products', array('rack' => $wh_qty['rack'],'quantity'=>$wh_qty['quantity']), array('product_id' => $id, 'warehouse_id' => $wh_qty['warehouse_id'])))
                    {


                        $items=$this->site->getPurchasedItems($id,$wh_qty['warehouse_id']);

                        if($items[0])
                        {
                            $this->db->update('purchase_items',array('quantity'=>$wh_qty['quantity'],'quantity_balance'=>$wh_qty['quantity']),array('id'=>$items[0]->id));
