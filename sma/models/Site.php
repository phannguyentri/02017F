...
class Site extends CI_Model
{
    ...
    public function getTaxRateByID($id) {
     ...
     public function getAllEnquiry1() {

        $q = $this->db->get_where('purchases', array('status' => 'approval'));
    ...
    public function getWarehouseItems($item_id, $warehouse_id = NULL) {
        if ($warehouse_id) {
            $this->db->where('warehouse_id', $warehouse_id);
        }
        $q = $this->db->get_where('warehouses_products', array('item_id' => $item_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function syncProductQty($product_id=NULL, $warehouse_id,$item_id=NULL) {
        ...
        if($item_id)
        {
            ...
            $balance_qty_item = $this->getBalanceItemQuantity($item_id);
            $wh_balance_qty_item = $this->getBalanceItemQuantity($item_id, $warehouse_id);

        if ($this->db->update('items', array('quantity' => $balance_qty_item), array('id' => $item_id))) {
            if ($this->getWarehouseItems($item_id, $warehouse_id)) {
                $this->db->update('warehouses_products', array('quantity' => $wh_balance_qty_item), array('item_id' => $item_id, 'warehouse_id' => $warehouse_id));
            } else {
                if( ! $wh_balance_qty_item) { $wh_balance_qty_item = 0; }
                $this->db->insert('warehouses_products', array('quantity' => $wh_balance_qty_item, 'item_id' => $item_id, 'warehouse_id' => $warehouse_id));
            }
                return TRUE;
            }
        }
    ...
    private function getBalanceItemQuantity($item_id, $warehouse_id = NULL) {
        $this->db->select('SUM(COALESCE(quantity_balance, 0)) as stock', False);
        $this->db->where('item_id', $item_id)->where('quantity_balance !=', 0);
        if ($warehouse_id) {
            $this->db->where('warehouse_id', $warehouse_id);
        }
        $q = $this->db->get('purchase_items');
    ...
    public function getPurchasedItems($product_id=NULL, $warehouse_id, $option_id = NULL,$item_id=NULL) {
    ...
    public function syncQuantity($sale_id = NULL, $purchase_id = NULL, $oitems = NULL, $product_id = NULL) {
        if ($sale_id) {
            ...
            foreach ($purchase_items as $item) {
                ...
                $this->syncProductQty($item->product_id, $item->warehouse_id,$item->item_id);


                if (isset($item->option_id) && !empty($item->option_id)) {
                    $this->syncVariantQty($item->option_id, $item->warehouse_id, $item->item_id);
                }
            }
    ...
    public function getAllProductionItems($sale_id) {
