...
class Enquiry_model extends CI_Model
{
    ...
    public function getProductNames($term, $limit = 5)
    {
        ...
        $this->db->where(" (item LIKE '%" . $term . "%' OR specification LIKE '%" . $term . "%' OR  concat(item, ' (', specification, ')') LIKE '%" . $term . "%')");
        $this->db->limit($limit);
        $q = $this->db->get('items');
        if ($q->num_rows() > 0) {
    ...
    {
        $this->db->select('purchase_items.*,items.specification, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant')
