...
if (localStorage.getItem('slitems')) {
	loadItems(billers, 'load_edit' , production_items);
}
...
if (slwarehouse = localStorage.getItem('slwarehouse')) {
	...
	// var old_row_qty;
	// function change_rquantity(billers){
...
//localStorage.clear();
function loadItems(billers = null, edit = null, production_items = null) {
	if (localStorage.getItem('slitems')) {
		total = 0;
		count = 1;
		an = 1;
		...
		$.each(slitems, function () {
			...
			// }
			tr_html += '<td class="text-center" style="width: 110px"><span class="text-right sprice quantity-config" id="sprice_' + row_no + '">' + item.row.quantity_config + '</span></td>';
			tr_html += '<td class="price" attr-price="'+item.row.price+'">'+formatMoney(item.row.price)+'</td>'
			tr_html += '<td class="weight">'+item.row.cf2+'</td>'
			tr_html += '<td style="width: 110px"><input class="form-control text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
			total_detail = item.row.quantity_config*item.row.qty;
			...
			// 	for(i in billers){
			// 		tr_html += '<option value="'+billers[i].id+'">'+ billers[i].name +'</option>'
			// 	}
			// 	tr_html +='</select>'
			// }else if(edit == 'load_edit'){
			// 	console.log(production_items);
			// 	for(i in production_items){
			// 		if (product_id == production_items[i].product_id) {
			// 		    var str = production_items[i].employees;
			// 		    var res = str.split(",");
			// 		    for(j in res){
			// 		    	for(k in billers){
			// 		    		if (res[j] == billers[k].id) {
			// 		    			tr_html += '<option value="'+billers[k].id+'">'+ billers[k].name +'</option>'
			// 		    		}
			// 		    	}
			// 		    }
			// 		}
			...
			if (edit == null) {
				tr_html += '<select name="employees_'+product_id+'[]" class="form-control select2_billers" multiple="multiple"> ';
				for(i in billers){
					tr_html += '<option value="'+billers[i].id+'">'+ billers[i].name +'</option>'
				}
				tr_html +='</select>'
				...
				for(i in production_items){
					if (product_id == production_items[i].product_id) {
						// console.log(production_items[i].product_id);
					    var str = production_items[i].employees;
					    if (production_items[i].employees) {
					    	current_bil = production_items[i].employees;
						tr_html += '<input value="'+current_bil+'"  name="employees_'+product_id+'[]" class="form-control select_billers" multiple="multiple"> ';
					    }else{
        ...
        $(".select_billers").select2("destroy").empty().attr("placeholder", "Chọn nhân viên").select2({
            placeholder: "Chọn nhân viên",
            data: arr_billers,
            multiple: true,

        });
        $(".select2_billers").select2();
 ...
 * @returns {Boolean}
 ---------------------------- */
 function add_invoice_item(item, billers = null, edit = null) {
