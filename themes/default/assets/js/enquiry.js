...
if (powarehouse = localStorage.getItem('powarehouse')) {
    ...
    /*$('.rexpiry').change(function (e) {
        ...
        poitemss[item_id].row.expiry = $(this).val();
...
$(document).on('click', '.podel', function () {
    ...
    if (count == 1) {
        ...
        //console.log(poitemss[item_id].row.name + ' is being removed.');
        delete poitemss[item_id];
        localStorage.setItem('poitemss', JSON.stringify(poitemss));
        row.remove();
                ...
                $.each(tax_rates, function () {
                    if(this.id == pr_tax){
                        if (this.type == 1) {
                            ...
                            if (poitemss[item_id].row.tax_method == 0) {
     ...
     * Edit Row Method 
     ...
     $(document).on('click', '#editItem', function () {
        ...
        poitemss[item_id].row.qty = parseFloat($('#pquantity').val()),
        poitemss[item_id].row.real_unit_cost = parseFloat($('#pcost').val()),
        poitemss[item_id].row.tax_rate = new_pr_tax,
        poitemss[item_id].tax_rate = new_pr_tax_rate,
        poitemss[item_id].row.discount = $('#pdiscount').val() ? $('#pdiscount').val() : '0',
        poitemss[item_id].row.option = $('#poption').val(),
        poitemss[item_id].row.expiry = $('#pexpiry').val() ? $('#pexpiry').val() : '';
        localStorage.setItem('poitemss', JSON.stringify(poitemss));
        $('#prModal').modal('hide');
        loadItems();
        return;
     ...
     * Edit Row Quantity Method 
     ...
     $(document).on("focus", '.rquantity', function () {
        ...
        item_id = row.attr('data-item-id');
        poitemss[item_id].row.qty = new_qty;
        localStorage.setItem('poitemss', JSON.stringify(poitemss));
        loadItems();
    });
     ...
     * Edit Row Cost Method 
     ...
     $(document).on("focus", '.rcost', function () {
        ...
        item_id = row.attr('data-item-id');
        poitemss[item_id].row.cost = new_cost;
        localStorage.setItem('poitemss', JSON.stringify(poitemss));
        loadItems();
    });
...
function loadItems() {
    ...
    if (localStorage.getItem('poitemss')) {
        ...
        total_discount = 0;
        $("#poTable tbody").empty();
        poitemss = JSON.parse(localStorage.getItem('poitemss'));

        $.each(poitemss, function () {

            var item = this;
            var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
            poitemss[item_id] = item;
           
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_cost = item.row.cost, item_qty = item.row.qty, item_bqty = item.row.quantity_balance, item_expiry = item.row.expiry, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_name = item.row.item,item_specification= item.row.specification;
