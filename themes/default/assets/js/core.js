...
$(document).ready(function() {
        ...
        $.ajax({type: "get", url: link, dataType: 'json',
            success: function(data) { 
                ...
                if(data['status'] == "success"){    
                    row.find('span.label.label-warning').html('Hủy bỏ');
                    row.find('span.label.label-warning').addClass('label-danger');
                    row.find('span.label.label-warning').removeClass('label-warning');
    ...
    $(document).on('click', '.po-update-status-production', function(e) {
        var row = $(this).closest('tr');
        e.preventDefault();
        $('.po').popover('hide');
        var link = $(this).attr('href');
        ...
        $.ajax({type: "get", url: link, dataType: 'json',
            success: function(data) { 
                ...
                if(data['status'] == "success"){
                    row.find('span.label.label-warning').html('Đang xử lý')                    
                    addAlert(data['msg'], 'success');
...
$(document).ready(function() {
    ...
    $('body').on('click', '.enquiry_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'enquiry/modal_view/' + $(this).parent('.enquiry_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'purchases/view/' + $(this).parent('.purchase_link').attr('id');
    });
    ...
    $('body').on('click', '.invoice_link td:not(:first-child :last-child)', function() {
     ...
     $('body').on('click', '.production_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'productions/modal_view/' + $(this).parent('.production_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.invoice_link').attr('id');
    });
